...
# --- Earth Engine Integration Note ---
st.markdown("""
---
### About the Data

**Current Implementation**: This demo uses simulated NDVI data that mimics typical patterns observed in Southeast Asian forests.

**Integration with Real Satellite Data**: To deploy with real data, this application can be connected to Google Earth Engine using the `geemap` or `ee` Python packages:
...
# Define function to calculate NDVI from Sentinel-2
def calculate_ndvi_sentinel2(start_date, end_date, aoi):
    ...
    ndvi_collection = sentinel.map(addNDVI)
